...
#### Phase 3D — DSAR admin UX, runbook, and end-to-end hardening

**Goal**: turn DSAR from backend capability into a usable, reviewable workflow for admins.

Deliverables:

- **Minimal admin UI (admin-only)**:
  - List: `GET /dashboard/settings/admin/dsar`
  - Detail: `GET /dashboard/settings/admin/dsar/:id`
  - Actions gated by status/type: approve/reject, export, restrict, erase/anonymize, mark failed, retry processing
  - Confirmation dialogs for destructive actions
- **Explicit workflow states and safe transitions**:
  - Backend rejects invalid transitions (`409`)
  - Export requires DSAR `APPROVED` and `requestType=EXPORT`
  - Restrict/erase execution requires DSAR `APPROVED` and correct `requestType`
- **Operator guidance embedded in UI**:
  - identity verification reminder
  - retention/legal-hold reminder for destructive actions
  - export content reminder (includes free-text + related records)
  - audit-trail reminder (IDs-only)
- **Operational documentation**:
  - `docs/DSAR_RUNBOOK.md`
  - `docs/DSAR_TEST_PLAN.md`
- **Hardening pass expectations**:
  - tenant isolation enforced in DSAR queries/execution
  - role checks are backend-enforced (`ADMIN`)
  - export format is stable and predictable
  - audit trail contains IDs/enums only (no names/emails/free-text)
  - no manual DB edits assumed for DSAR execution
