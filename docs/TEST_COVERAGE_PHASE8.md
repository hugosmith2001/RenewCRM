...
- **401/403**: When `requireRole` rejects with `"Unauthorized"` or `"Forbidden"`, the handler returns 401 or 403 and does not call `listAuditEvents`.
...
- **Roles**: Both ADMIN and BROKER are allowed (via `requireRole([Role.ADMIN, Role.BROKER])`); the test only asserts BROKER can get 200.
...
- **Real auth/session**: No real NextAuth session or JWT; `requireRole` is mocked.
- **STAFF and audit list**: There is no test that STAFF is **forbidden** from GET /api/audit (the route uses `requireRole([Role.ADMIN, Role.BROKER])`, so STAFF would get 403 from `requireRole`; this is implicit).
...
| **GET /api/audit** | STAFF role | Not tested; requireRole([ADMIN, BROKER]) implies STAFF gets 403. |
...
| `app/api/audit/route.test.ts` | GET /api/audit: auth, validation, success, query forwarding, BROKER access. |
